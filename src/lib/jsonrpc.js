...
const log = require('../lib/logger')({ name: 'jsonrpc.js ' });

const HOSTNAME = os.hostname();
const URL = process.env.RIPPLED_ENV === 'SELF'?'http://127.0.0.1:5005':'https://xrplcluster.com';
const URL_HEALTH = process.env.RIPPLED_ENV === 'SELF'?'http://127.0.0.1:5005/health':'https://xrplcluster.com/health';
...
// Get ledger data
module.exports.getLedgerData = async (params) => {
    return query({
        method: 'ledger_data',
        params: [{
            ...params
...
// Get ledger
module.exports.getLedgerSimple = (ledger_index) => {
    return query({
        method: 'ledger',
...
module.exports.getOffers = (issuer, currencyCode, pairCurrencyCode, pairIssuerAddress) => {
    // SOLO, rsoLo2S1kiGeCcn6hCUXVrCpGMWLrRrLZz, XRP, undefined
    return query({
        method: 'book_offers',
        params: [{
            taker_gets: {
                currency: `${currencyCode.toUpperCase()}`,
                issuer: currencyCode.toUpperCase() === 'XRP' ? undefined : `${issuer}`,
            },
            taker_pays: {
